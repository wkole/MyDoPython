...
def mymain():
    # createPlot()
    retTree = retrieveTree(1)
    myTree = retrieveTree(0)
    print retTree
    print myTree
    print myTree.keys()[0]
    print myTree[myTree.keys()[0]]
    print myTree[myTree.keys()[0]].keys()
...
def mymain():
    ...
    # 用MyTestTree中数据集画的分类树
    print StorAndRead.grabTree('cFstorage.txt')
    createPlot(StorAndRead.grabTree('cFstorage.txt'))

    # 用本MyTestTreePlotter中的测试数据集画的（可以独立运行）
    # createPlot()#这个和代码中第一个createplot()
    # retTree = retrieveTree(1)
    # myTree = retrieveTree(0)
    # print retTree
    # print myTree
